...
class Producto(models.Model):  # TODO Explicar para que es el models.Model
    ...
    )  # cambiamos a many2many para que peuda tener varias categorias
    ...
    activo = models.BooleanField(
        default=True, help_text="Si se desactiva, no se usará en nuevas cajas."
    )
...
class Mystery_Box(models.Model):
    ...
    estado = models.CharField(
        max_length=20, choices=(("A", "Activa"), ("I", "Inactivo")), default="A"
    )
    OPCIONES_DESCUENTO = [
        (0, "Sin Descuento"),
    ...
    def precio_suscripcion(self):  #! precio para suscriptores
        ...
        descuento = self.precio_base * (
            Decimal(self.porcentaje_descuento) / Decimal(100)
        )
...
class Suscripcion(models.Model):
    usuario = models.OneToOneField(
        User, related_name="membresia", on_delete=models.PROTECT
    )
    estado = models.CharField(
        max_length=20,
        choices=(
...
class Envio(models.Model):
    usuario = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="mis_envios"
    )
    caja = models.ForeignKey("Mystery_Box", on_delete=models.PROTECT)
    #! LOGISTICA DE ENVIO
    fecha_envio = models.DateTimeField(
        default=timezone.now
    )  # investigar auto_now_add=True
    ...
    estado = models.CharField(
        choices=[("P", "Preparando caja"), ("E", "Enviado"), ("R", "Recibido")],
        default="P",
    )
    ...
    )  # El IVA que se aplicó (snapshot)
    monto_impuesto = models.DecimalField(
        max_digits=10, decimal_places=2, default=0
    )  # El dinero cobrado de IVA
    valor_total = models.DecimalField(
        max_digits=10, decimal_places=2, default=0
    )  # Subtotal + Impuestos
    valor_pagado = models.DecimalField(
        max_digits=10, decimal_places=2, default=0
    )  # Total pagado real (revenue)
    costo_contenido = models.DecimalField(
        max_digits=10, decimal_places=2, default=0
    )  # Costo de los items generados
    stripe_session_id = models.CharField(
        max_length=255, blank=True, null=True
    )  # ID de sesión de Stripe para referencia
    codigo_rastreo_interno = models.CharField(
        max_length=50, blank=True, null=True
    )  # ointerno
    numero_guia = models.CharField(
        max_length=50, blank=True, null=True
    )  # servientrega o fedex
...
class Configuracion(models.Model):
    ...
    tasa_iva = models.DecimalField(
        ...
        decimal_places=2,
        default=15.00,
        help_text="Porcentaje de IVA actual (ej. 15.00)",
    )
...
class CarritoItem(models.Model):  #! modelo para los items del carrito de compra
    usuario = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="carrito_items"
    )
    caja = models.ForeignKey("Mystery_Box", on_delete=models.CASCADE)
    cantidad = models.PositiveIntegerField(default=1)
    fecha_agregado = models.DateTimeField(
        auto_now_add=True
    )  # fecha en la que se agrego al carrito

    class Meta:
        unique_together = (
            "usuario",
            "caja",
        )  # Un usuario no puede tener la misma caja duplicada en el carrito, se sumara luego
...
class Cupon(models.Model):
    ...
    descuento_porcentaje = models.IntegerField(
        default=0, help_text="Descuento en porcentaje (0-100)"
    )
    descuento_fijo = models.DecimalField(
        max_digits=10,
        decimal_places=2,
        default=0,
        help_text="Descuento fijo en dólares",
    )
    activo = models.BooleanField(default=True)
    fecha_inicio = models.DateTimeField()
    fecha_expiracion = models.DateTimeField()
    usos_maximos = models.IntegerField(
        null=True, blank=True, help_text="Dejar vacío para usos ilimitados"
    )
    usos_actuales = models.IntegerField(default=0)
    solo_premium = models.BooleanField(
        default=False, help_text="Solo para usuarios premium"
    )
    monto_minimo = models.DecimalField(
        max_digits=10, decimal_places=2, default=0, help_text="Monto mínimo de compra"
    )
    ...
    def es_valido(self, usuario=None, monto_compra=0):
        ...
        if ahora > self.fecha_expiracion:
            return (
                False,
                f"Este cupón expiró el {self.fecha_expiracion.strftime('%d/%m/%Y a las %H:%M')}",
            )
...
class CuponUso(models.Model):
    usuario = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="cupones_usados"
    )
