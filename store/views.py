...
from django.http import JsonResponse, HttpResponse  # Para respuestas AJAX
...
def box(
    request, id
):  # COMO LAS CAJAS RECIBEN UN ID PARA VER QUE HACER CON EL HTML PONEMOS ACA EL ID
...
class ProductoListView(LoginRequiredMixin, ListView):
    queryset = Producto.objects.prefetch_related(
        "categoria"
    )  # prefetch related es para many to many
...
class SuscripcionListView(
    LoginRequiredMixin, ListView
):  # todo esto ed envios global, queremos que cada usuario pueda ver su propio historial
    model = Suscripcion
    ...
    def get_queryset(self):  # probar este luego
        ...
        if usuario_actual.has_perm(
            "store.view_suscripcion"
        ):  # TODO en los modelos crear permisos personalizados para lo que ocupemos
...
def crear_checkout_session(request, id):  #! EStE STRIPE ES PARA COMPRAS NORMALES
    ...
    if caja.es_exclusiva and not es_premium:
        return redirect(
            "pagina_hazte_premium"
        )  # Mandarlo a que se suscriba si la caja es premium

    if (
        caja.estado != "A"
    ):  # validar que sea activa, sino con el link nos hacen la 13 14
    ...
    try:
        ...
        checkout_session = stripe.checkout.Session.create(
            payment_method_types=[
                "card"
            ],  # tipo de pago, hay mas como cripto por ejemplo
            #! VAmos a pedir la direccion de envio tambien
            shipping_address_collection={
                "allowed_countries": ["EC"],
            },
            client_reference_id=str(request.user.id),
            metadata={"tipo_compra": "unica", "caja_id": caja.id},
            line_items=[  # el carrito de compras
                {
                    ...
                    "price_data": {  # esto son los datos de los precios
                        ...
                        "unit_amount": int(
                            precio_final * 100
                        ),  # Stripe usa centavos (25.00 -> 2500)
...
def compra_exitosa(request, id):
    ...
    # Note: Fulfillment is now handled via Stripe Webhooks for security
    return render(
        request,
        "store/templates/pago_exitoso.html",
        {"caja": caja, "processing": True},
    )
...
def confirmar_recepcion(request, pk):
    ...
    if envio.estado == "E":
        ...
        messages.success(
            request,
            f"¡Gracias! Has confirmado la recepción del pedido {envio.codigo_rastreo_interno}.",
        )
...
class EnviosListView(LoginRequiredMixin, PermissionRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        return (
            Envio.objects.filter(estado__in=["P", "E"])
            .select_related("usuario", "caja")
            .order_by("fecha_envio", "codigo_rastreo_interno")
        )
...
class EnviosUpdateView(LoginRequiredMixin, PermissionRequiredMixin, UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        hermanos = Envio.objects.filter(
            codigo_rastreo_interno=envio.codigo_rastreo_interno
        )
    ...
    def form_valid(self, form):
        ...
        if (
            envio.estado == "E"
            and not envio.numero_guia
            and "estado" in form.changed_data
        ):
            form.add_error(
                "numero_guia", "Debes ingresar una guía para marcar como Enviado"
            )
...
def rastrear_pedido(request):
    ...
    if guia:
        try:
            ...
            envios = Envio.objects.filter(codigo_rastreo_interno=guia).select_related(
                "usuario", "caja"
            )
    ...
    return render(
        request,
        "store/templates/rastreo.html",
        {
            "envio": envio,  # Mantenemos compatibilidad con template actual (toma el primero)
            "envios_lista": envios
            if "envios" in locals() and envios.exists()
            else None,  # Lista completa
            "error": error,
            "guia_buscada": guia,
        },
    )
...
def perfil(request):
    ...
    mis_envios = (
        Envio.objects.filter(usuario=request.user)
        .select_related("caja")
        .order_by("-fecha_envio", "codigo_rastreo_interno")
    )
    ...
    return render(
        ...
        {"mis_envios": mis_envios, "suscripcion": suscripcion},
    )
...
def membresia_exitosa(request):
    ...
    suscripcion, created = Suscripcion.objects.get_or_create(
        ...
        defaults={
            "estado": "I"
        },  # si es nueva inicia en inactiva, esto para el paso de mas adelante
    )
    ...
    if (
        ...
        and suscripcion.fecha_proximo_pago
        and suscripcion.fecha_proximo_pago > hoy
    ):
...
def agregar_al_carrito(request, id):
    ...
    if caja.estado != "A":
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            return JsonResponse(
                {"success": False, "error": "Esta caja no está disponible"}
            )
        return redirect("home")
    ...
    if caja.es_exclusiva and not es_usuario_premium(request.user):
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            return JsonResponse(
                {
                    "success": False,
                    "error": "Esta caja es exclusiva para socios premium",
                }
            )
...
def ver_carrito(request):
    ...
    cupon_descuento = Decimal(str(request.session.get("cupon_descuento", 0))).quantize(
    ...
    iva_calculado = (subtotal_con_descuento * (tasa_iva / Decimal(100))).quantize(
        cents, rounding=ROUND_HALF_UP
    )

    # 5. Total Final
    total_final = (subtotal_con_descuento + iva_calculado).quantize(
        cents, rounding=ROUND_HALF_UP
    )
...
def checkout_carrito(request):
    ...
    try:
        ...
        # Create a simplified version of the cart to store in metadata
        cart_data = [{"id": item["caja"].id, "qty": item["cantidad"]} for item in items]
        ...
        checkout_session = stripe.checkout.Session.create(
            ...
            metadata={
                ...
                "cart_data": json.dumps(cart_data)[
                    :500
                ],  # Stripe metadata max len is 500
                "cupon_codigo": request.session.get("cupon_codigo", ""),
            },
...
def compra_exitosa_carrito(request):
    ...
    return render(
        request,
        "store/templates/pago_exitoso_carrito.html",
        {
            "processing": True,
        },
    )
...
def validar_cupon(request):
    ...
    if request.method == "POST":
        ...
        es_valido, mensaje = cupon.es_valido(
            request.user if request.user.is_authenticated else None, subtotal
        )
        ...
        messages.success(
            request, f"✅ Cupón {codigo_cupon} aplicado: ${descuento} de descuento"
        )
        return redirect("ver_carrito")
    return redirect("ver_carrito")
...
class CuponCreateView(LoginRequiredMixin, PermissionRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        messages.success(
            self.request, f"✅ Cupón {form.instance.codigo} creado exitosamente"
        )
        return super().form_valid(form)
...
class AuditLogView(LoginRequiredMixin, PermissionRequiredMixin, ListView):
        ...
        if hasattr(Envio, "history"):
            for record in Envio.history.all()[:50]:
                logs.append(
                    {
                        "model": "Envío",
                        "object": record.codigo_rastreo_interno
                        or f"Envío #{record.id}",
                        "action": self.get_action_display(record.history_type),
                        "user": record.history_user,
                        "date": record.history_date,
                        "changes": self.get_changes(record),
    ...
    def get_action_display(self, history_type):
        actions = {"+": "Creado", "~": "Modificado", "-": "Eliminado"}
...
class UserListView(LoginRequiredMixin, UserPassesTestMixin, ListView):
    ...
    def test_func(self):
        return (
            ...
            or self.request.user.groups.filter(name="Administrador").exists()
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            ...
            context["cupones_usados"] = (
                CuponUso.objects.filter(usuario=user)
...
# Webhook para procesar el pago desde Stripe de forma segura
@csrf_exempt
def stripe_webhook(request):
    payload = request.body
    sig_header = request.META.get("HTTP_STRIPE_SIGNATURE")

    # Intenta obtener la clave de webhook de settings
    endpoint_secret = getattr(settings, "STRIPE_WEBHOOK_SECRET", None)

    if not endpoint_secret:
        return HttpResponse("STRIPE_WEBHOOK_SECRET no configurado", status=400)
...
def _handle_successful_payment(session):
    ...
    # Evitamos duplicados buscando el session_id
    if Envio.objects.filter(stripe_session_id=session_id).exists():
        return

    # Obtener detalles de envío
    datos_envio = session.get("shipping_details") or session.get("customer_details")
    nombre_cliente = datos_envio.get("name") if datos_envio else user.username
    info_direccion = datos_envio.get("address", {}) if datos_envio else {}
    ...
    codigo_rastreo = generar_id_interno()
    tasa_iva = Configuracion.get_iva()
    es_premium = es_usuario_premium(user)

    if tipo_compra == "unica":
        caja_id = metadata.get("caja_id")
        if not caja_id:
            return
        try:
        ...
        precio_unitario = caja.precio_suscripcion if es_premium else caja.precio_base
        ...
        nuevo_envio = Envio.objects.create(
            ...
            codigo_rastreo_interno=codigo_rastreo,
            estado="P",
            subtotal=precio_unitario,
            iva_porcentaje=tasa_iva,
            monto_impuesto=iva_item,
            valor_total=precio_unitario + iva_item,
        )
        generar_caja(nuevo_envio)

    elif tipo_compra == "carrito":
        cart_data_str = metadata.get("cart_data", "[]")
        try:
            items = json.loads(cart_data_str)
        except:
            items = []
        ...
        for item in items:
            ...
            precio_unitario = (
                caja.precio_suscripcion if es_premium else caja.precio_base
            )
            iva_item = precio_unitario * (tasa_iva / Decimal(100))
        ...
        cupon_codigo = metadata.get("cupon_codigo")
        if cupon_codigo:
            try:
                ...
                cupon.usar()
                CuponUso.objects.create(usuario=user, cupon=cupon, descuento_aplicado=0)
            except:
                pass
