...
from decimal import (
    Decimal,
)  # para la multiplicacion, para que no tenga asi de que 1.000000001
...
def generar_caja(Envio_obj):  # recibe un objeto de envio, vacio y lo llena
    ...
    productos_posibles = list(
        Producto.objects.select_for_update().filter(
            categoria__in=caja.allowed_categories.all(), activo=True, stock__gt=0
        )
    )  # genera una lista de los productos posibles

    if (
        not productos_posibles
    ):  # SI NO HAY PRODUCTOS DE ESA CATEGORIA USAMOS DE CUALQUIERA COMO PLAN B
        # TODO que de un error en lugar de
        productos_posibles = list(
            Producto.objects.select_for_update().filter(activo=True, stock__gt=0)
        )
    ...
    valor_acumulado = Decimal(
        "0.00"
    )  # aca es donde vamos a comoparar para ver que no nos quedemos pobres con la caja kakakakka

    random.shuffle(
        productos_posibles
    )  # hacemos un random de todos los productos posibles. que rico es el sexo por telepatia con enanos
    ...
    for producto in productos_posibles:
        ...
        if (
            (valor_acumulado + producto.valor) <= valor_maximo
        ):  # revisa si al meter este producto nos pasamos del presupuesto ya no lo mete, sino si
            productos_seleccionados.append(
                producto
            )  # va a llenar productos hasta que tenga 3 o mas y ahi se acaba #todo que pasa si ya no hay
    ...
    if len(productos_seleccionados) < 3:
        ...
        faltantes = (
            3 - len(productos_seleccionados)
        )  # // todo hay un error y es que al rellenar si es que falta con productos aleatorios sin filtrar en la categoria puede repetir objeto
        ...
        )  # esto es como lo del codewars, es el mismo cidigo que podrias hacer hacia abajo pero comprimido
        ...
        cantidad_k = min(
            faltantes, len(items_reserva)
        )  # k es la variable del sample, agarramo el faltantes o si hay menos pues menos
        if (
            cantidad_k > 0
        ):  # osea si tenemos items de reserva, sino pues ya sobate no hay nada base de datos para mala
            relleno = random.sample(items_reserva, cantidad_k)

            productos_seleccionados.extend(relleno)
            for producto in relleno:
                producto.stock -= 1
                producto.save()
    ...
    Envio_obj.valor_pagado = Envio_obj.valor_total
    Envio_obj.costo_contenido = total_precio
